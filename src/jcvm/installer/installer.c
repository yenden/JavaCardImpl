...
bool installHeaderComp(CardApplet *newApplet){
    ...
    iPos += 6;
    HeaderComponent *ipHeader =  &newApplet->absApp.pHeader;
...
bool installDirComp(CardApplet *newApplet){
    uint16_t iPos = 0;
    DirectoryComponent *ipDir = &newApplet->absApp.pDir ;
...
void installAppletComp(CardApplet *newApplet){
    ...
    AppletComponenent *ipAppletComponent = &newApplet->pApplet;
    iPos += 3;
    if (appletCompSize !=0) {
...
bool installImportComp(CardApplet *newApplet){
    ...
    ImportComponent *ipImportComponent = &newApplet->absApp.pImport;
    if (importCompSize ==0) {
...
void installClassComp(CardApplet *newApplet){
    ...
    ClassComponent *ipClassComponent = &newApplet->absApp.pClass;
...
void installConstPoolComp(CardApplet *newApplet){
    uint16_t iPos = 3;
    ConstantPoolComponent *ipConstPoolComponent = &newApplet->absApp.pConstantPool;
...
void installStaticFieldComp(CardApplet *newApplet){
    uint16_t iPos = 3;
    StaticFieldComponent *ipStaticFieldComponent = &newApplet->absApp.pStaticField;
...
void installMethodComp(CardApplet *newApplet){
    uint16_t iPos = 3;
    MethodComponent *ipMethodComp = &newApplet->absApp.pMethod;
...
void installExportComponent(CardApplet *newApplet){
    uint16_t iPos = 3;
    ExportComponent *ipExportComp = &newApplet->absApp.pExport;
...
void installDescriptorComp(CardApplet *newApplet){
    uint16_t iPos = 3;
    DescriptorComponent *ipDescComp = &newApplet->absApp.pDescriptor;
    ...
    ipDescComp->classCount = readU1(descriptorComp, &iPos);
     for(uint16_t i=0;i<ipDescComp->classCount;i++){
        ...
        for(uint16_t u=0; u < ipDescComp->classes[i].methodCount; u++){
        	ipDescComp->classes[i].methods[u].token = readU1(descriptorComp, &iPos);
        	ipDescComp->classes[i].methods[u].pAF = readU1(descriptorComp, &iPos);
        	ipDescComp->classes[i].methods[u].methodOffset = readU2(descriptorComp, &iPos);
        	ipDescComp->classes[i].methods[u].typeOffset = readU2(descriptorComp, &iPos);
        	ipDescComp->classes[i].methods[u].bytecodeCount = readU2(descriptorComp, &iPos);
        	ipDescComp->classes[i].methods[u].exceptionHandlerCount = readU2(descriptorComp, &iPos);
        	ipDescComp->classes[i].methods[u].exceptionHandlerIndex = readU2(descriptorComp, &iPos);
        }
    }
